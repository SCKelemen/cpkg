...
For each dependency module in `cpkg.lock.yaml`, `cpkg sync` guarantees:
...
1. **Deterministic dependencies**
   ...
   * Every project has a manifest (`cpkg.yaml`) and a lockfile (`cpkg.lock.yaml`).
...
### 2.2 `cpkg.lock.yaml` — Lockfile

**File name:** `cpkg.lock.yaml`

Generated by `cpkg tidy`. MUST NOT be edited manually.

```yaml
# cpkg.lock.yaml — generated, do not edit by hand.
...
* Run from project root (directory containing `cpkg.yaml`), or walk upward until found.
* Exit status:
...
**Purpose:** Resolve dependency graph and write `cpkg.lock.yaml`.
...
**Purpose:** Make working tree (git submodules) match `cpkg.lock.yaml`.
...
* Ensure `cpkg.lock.yaml` exists; if not, run `cpkg tidy` first.
...
* Compare `cpkg.yaml` and `cpkg.lock.yaml`.
* For each dependency:
...
* Read `cpkg.yaml` and `cpkg.lock.yaml`.
* For each dependency:
...
* Internal packages (suggested):
  ...
  * `internal/lockfile` — parse & write `cpkg.lock.yaml`.
